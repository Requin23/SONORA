...
export default function ClientShell({ path }: Props) {
  ...
  async function checkout(orderId: string, method: string, provider: "yengapay" | "paydunya" = "yengapay") {
    setMessage(`Initialisation du paiement (${provider === "paydunya" ? "PayDunya" : "YengaPay"})...`);
    const response = await fetch(`/api/orders/${orderId}/checkout`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ paymentMethod: method, provider }),
    });
    const data = (await response.json()) as { paymentUrl?: string; error?: string };
    if (!response.ok || !data.paymentUrl) {
      setMessage(data.error ?? "Impossible d'initialiser le paiement.");
      return;
    }
  ...
  return (
    <section className="section">
      ...
      <div className="form-grid">
        ...
        {props.step === 4 && <><Input label="Duree souhaitee en secondes" value={String(props.form.dureeSouhaitee ?? 120)} onChange={(value) => setField("dureeSouhaitee", Number(value))} /><Input label="Deadline souhaitee" value={props.form.deadline ?? ""} onChange={(value) => setField("deadline", value)} /><Input label="WhatsApp (optionnel, pour recevoir le lien)" value={props.form.whatsapp ?? ""} onChange={(value) => setField("whatsapp", value)} /></>}
...
function Payment({ order, checkout, simulatePaymentWebhook }: { order: Order; checkout: (id: string, method: string, provider: "yengapay" | "paydunya") => void; simulatePaymentWebhook: (id: string) => void }) {
  const devTools = process.env.NEXT_PUBLIC_ENABLE_DEV_TOOLS === "true";
  return (
    <section className="section">
      <h1>Paiement</h1>
      <p>Commande {order.id} - {formatPrice(order.price)}</p>
      <p>Choisissez votre moyen de paiement local :</p>
      <div className="actions">
        <button className="button primary" onClick={() => checkout(order.id, "orange_money", "paydunya")}>Orange Money</button>
        <button className="button" onClick={() => checkout(order.id, "wave", "paydunya")}>Wave</button>
        <button className="button" onClick={() => checkout(order.id, "card", "paydunya")}>Carte bancaire</button>
        <button className="button" onClick={() => checkout(order.id, "mobile_money", "yengapay")}>Autres (YengaPay)</button>
        {devTools && (
...
function AdminOrderRow({
  ...
  return (
    <article className="admin-row">
      ...
      {order.deliverables.length > 0 && order.requestForm?.whatsapp && (() => {
        const last = order.deliverables[order.deliverables.length - 1];
        const message = `Bonjour ! Votre chanson Sonora est prete 🎵\nCommande ${order.id}\nTelechargez votre fichier (${last.format.toUpperCase()}) ici : ${last.fileUrl}`;
        return (
          <a className="button" href={waMeLink(order.requestForm.whatsapp ?? "", message)} target="_blank" rel="noreferrer">
